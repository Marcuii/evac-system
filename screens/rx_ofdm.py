...
class rx_ofdm(gr.top_block):
    ...
    def __init__(self):
        ...
        self.central_freq = central_freq = 2.4e9
        ...
        self.blocks_head_0 = blocks.head(gr.sizeof_char*1, 7000)
        self.blocks_file_sink_0 = blocks.file_sink(gr.sizeof_char*1, 'data_rx.json', False)
        ...
        self.connect((self.digital_crc32_bb_0, 0), (self.blocks_head_0, 0))
...
def main(top_block_cls=rx_ofdm, options=None):
    ...
    # Monitor thread to stop flowgraph when head block is done
    def monitor_head_block():
        """Wait for head block to finish, then stop the flowgraph"""
        while True:
            time.sleep(0.5)
            # Check if head block has processed all items (nitems_written >= limit)
            try:
                items_written = tb.blocks_head_0.nitems_written(0)
                if items_written >= 7000:
                    print(f"[rx_ofdm] Received {items_written} bytes, stopping...")
                    time.sleep(0.5)  # Allow file sink to flush
                    tb.stop()
                    break
            except:
                break

    monitor_thread = threading.Thread(target=monitor_head_block, daemon=True)
    monitor_thread.start()

    tb.wait()
    print("[rx_ofdm] Flowgraph terminated successfully")
